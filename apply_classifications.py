...
Usage:
  ...
  python3 apply_classifications.py --data app/data.json --in vehicle_classification_template.csv
"""
...
def main():
    ...
    if invalid:
        print("\\nInvalid class values (not in vehicleClasses):")
        for k,v in invalid[:50]:
            print(f" - {k}: {v}")
        if len(invalid)>50:
            print(f" ... +{len(invalid)-50} more")
    if missing:
        print("\\nRows that did not match any question (check id/name):")
        for k in missing[:50]:
            print(f" - {k}")
        if len(missing)>50:
